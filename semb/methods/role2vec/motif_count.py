...
class MotifCounterMachine(object):
    ...
    def create_edge_subsets(self):
        ...
        unique_subsets = dict()
        for i in range(3,self.params['graphlet_size']+1):
    ...
    def enumerate_graphs(self):
        ...
        self.interesting_graphs = {i:[] for i in range(2,self.params['graphlet_size']+1)}
        for graph in graphs:
            if graph.number_of_nodes() > 1 and  graph.number_of_nodes() < self.params['graphlet_size']+1:
    ...
    def create_tabular_motifs(self):
        ...
        for index in range(self.unique_motif_count):
            ...
            if sum(features)>0:
                ...
                features = pd.qcut(features,self.params['quantiles'],duplicates="drop", labels=False)
                for node in self.graph.nodes():
                    self.binned_features[node].append(str(int(index*self.params['quantiles'] + features[node])))
    ...
    def factorize_string_matrix(self):
        ...
        model = NMF(n_components=self.params['factors'], init='random', random_state=self.params['seed'], alpha = self.params['beta'])
        factors = model.fit_transform(features)
        kmeans = KMeans(n_clusters=self.params['clusters'], random_state=self.params['seed']).fit(factors)
    ...
    def create_string_labels(self):
        ...
        if self.params['motif_compression'] == "string":
