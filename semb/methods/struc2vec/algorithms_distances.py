...
def generate_distances_network(workers):
    ...
    t0 = time()
    os.system("rm "+returnPickle().replace(' ', '\ ')+"graphs-layer-*.pickle")
    with ProcessPoolExecutor(max_workers=1) as executor:
    ...
    t0 = time()
    os.system("rm "+returnPickle().replace(' ', '\ ')+"distances_nets_weights-layer-*.pickle")
    os.system("rm "+returnPickle().replace(' ', '\ ')+"alias_method_j-layer-*.pickle")
    os.system("rm "+returnPickle().replace(' ', '\ ')+"alias_method_q-layer-*.pickle")
    with ProcessPoolExecutor(max_workers=1) as executor:
