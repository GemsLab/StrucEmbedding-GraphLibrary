...
from .graphwave_utils.graph_tools import laplacian #MH slight edit--change utils to graphwave_utils to avoid name clash


ORDER = 30#30 #MH changed for scalability
PROC = 'approximate'
ETA_MAX = 0.95
ETA_MIN = 0.80
NB_FILTERS = 2
...
def heat_diffusion_ind(graph, taus, order = ORDER, proc = PROC):
    '''
    ...
    if proc == 'exact':
        ...
        for i in range(n_filters):
             heat[i] = sc.sparse.csc_matrix(U.dot(np.diagflat(np.exp(- taus[i] * lamb).flatten())).dot(U.T)) #MH seemed to be a bug--need to convert to array
    else:
