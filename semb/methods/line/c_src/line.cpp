...
namespace LINE {
	...
	real * n, *emb_context, *sigmoid_table;
	...
	{
		/**
		 * TODO: Replace below with C++ strings passed from outside
		 */
		FILE *fin;
		...
		num_edges = 0;
		while (fgets(str, sizeof(str), fin)) num_edges+=2;
		fclose(fin);
		...
		for (int k = 0; k != num_edges / 2; k++)
		{
			...
			vertex[vid].degree += 2;
			edge_source_id[2 * k] = vid;
			edge_target_id[2 * k + 1] = vid;

			vid = SearchHashTable(name_v2);
			if (vid == -1) vid = AddVertex(name_v2);
			vertex[vid].degree += 2;
			edge_target_id[2 * k] = vid;
			edge_source_id[2 * k + 1] = vid;
	...
	void TrainLINE() {
		...
		printf("ReadData();\n");
		InitAliasTable();
		printf("InitAliasTable();\n");
		InitVector();
		printf("InitVector();\n");
		InitNegTable();
		printf("InitNegTable();\n");
		InitSigmoidTable();
		printf("InitSigmoidTable();\n");
		gsl_rng_env_setup();
		printf("gsl_rng_env_setup();\n");
